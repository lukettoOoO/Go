...
// go's approach makes it easy to see which functions reutrn errors and to handle them using the same language constructs employed for other, non-error tasks
package main
...
func makeTea(arg int) error {
	if arg == 2 {
		...
		// wrapped errors create a logical chain (A wraps B, which wraps C, etc.) that can be queried with functions like errors.Is and errors.AsType
...
func main() {
	...
	for i := range 5 {
		if err := makeTea(i); err != nil {
			...
			// this is especially usefull with wrapped or nested errors, allowing you to identify specific error types or sentinel errors in a chain of errors
