...
func main() {
	...
	// this is common pattern to prevent goroutine leaks in case the channel is never read
	...
	// res := <-c1 awaits the result and <-time.After awaits a value to be sent after the timesot of 1s
	// since select proceeds with the first receive that's ready, we'll take the timeout case if the operation takes more than the allowrd 1s
	select {
	case res := <-c1:
		fmt.Println(res)
